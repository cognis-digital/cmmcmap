"""Native cognis-connect emit for cmmcmap — forward findings to any platform.

Maps cmmcmap's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    cmmcmap ... --format json | cmmcmap-emit --to stix
    cmmcmap-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        if 'title' in rec: out['title'] = rec['title']
        if 'severity' in rec: out['severity'] = rec['severity']
        if 'type' in rec: out['type'] = rec['type']
        if 'description' in rec: out['description'] = rec['description']
        if 'tags' in rec: out['tags'] = rec['tags']
        if 'ipv4' in rec: out['ipv4'] = rec['ipv4']
        if 'domain' in rec: out['domain'] = rec['domain']
        if 'url' in rec: out['url'] = rec['url']
        if 'sha256' in rec: out['sha256'] = rec['sha256']
        if 'cve' in rec: out['cve'] = rec['cve']
        if 'imo' in rec: out['imo'] = rec['imo']
        if 'mmsi' in rec: out['mmsi'] = rec['mmsi']
        if 'lat' in rec: out['lat'] = rec['lat']
        if 'lon' in rec: out['lon'] = rec['lon']
        return out
    except Exception:
        return rec
